  ...
  return (
    <div className='container'>
      <div style={hideWhenVisible}>
        <button onClick={toggleVisibility} className="btn btn-primary" id={props.idShow}>{props.buttonLabel}</button>
      </div>
      <div style={showWhenVisible}>
        {props.children}
        <button onClick={toggleVisibility} className="btn btn-secondary" id={props.idCancel}>cancel</button>
      </div>
    </div>
  );
});
