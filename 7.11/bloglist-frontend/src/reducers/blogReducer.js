...
const createBlog = (token, blog) => {
  return async (dispatch) => {
    blogService
      ...
      .catch((data) => dispatch(setNotification({ message: `Error creating blog ${blog.title}` + data, type: 'error' }, 5)));
  };
};
...
const likeBlog = (token, id) => {
  return async (dispatch) => {
    blogService.getByID(token, id).then((blog) => {
      ...
      blogService.updateBlog(token, newBlog, id).then(() => {
        dispatch(addLike(id));
        dispatch(setNotification({ message: 'You liked ' + newBlog.title, type: 'success' }, 10));
      }).catch((data) => dispatch(setNotification({ message: `Error updating blog ${blog.title}` + data, type: 'error' }, 5)));
    });
  };
};
...
export const { addLike, addBlog, appendBlog, setBlogs, removeBlog } = blogSlice.actions;
export { getId, initializeBlogs, createBlog, likeBlog, eraseBlog };
